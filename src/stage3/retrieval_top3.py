...
def search_top_k(index: faiss.IndexFlatIP, query_vec: np.ndarray, top_k: int):
    ...
    return scores[0].tolist(), [int(x) for x in ids[0].tolist()]
...
def run_query(
    ...
    content_scores, content_ids = search_top_k(content_index, query_content, top_k)
    voice_scores, voice_ids = search_top_k(voice_index, query_voice, top_k)
